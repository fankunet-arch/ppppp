...
final class PointsEngine
{
    ...
    public static function validateAllocations(
        ...
        foreach ($allocations as $a) {
            ...
            /**
             * ★ 「0 元 0 份」这一行没有任何意义，直接拒（审计 F10）。
             *
             *   它落库之后是一条【0 元 0 分 0 次】的流水，而那条流水
             *   会把这位客人永久锁死在这张单上：想补记 →
             *   member_already_on_order；想撤销 → 撤的是钱和次，
             *   撤一条全零的等于没撤。客人只能等订单过了保护期。
             *
             *   原来只有【整笔全零】才被拒（下面那条 empty_allocation），
             *   混在多人提交里的单独一条零行是通得过的。
             *
             *   ★ 说清楚现状，免得下一个人误判能不能放松这条：
             *     Pad 在提交前【已经滤掉】零行
             *     （pad.js: filter(p => p.amountCents > 0 || p.portions > 0)），
             *     所以走 Pad 的正常流程撞不到这里。
             *     这一条守的是【绕过 Pad 的调用方】—— 集成、脚本、
             *     以及将来任何一个忘了照抄那个 filter 的新客户端。
             *     服务端不能把「客户端会先滤一遍」当成前提，
             *     那正是 docs/13 §3.4 里「碰巧存在」的同一类错误。
             *
             *   ★ splitEvenly 在「钱比人少」时确实会拆出零行
             *     （实测 3 分 2 份分给 4 人 → 后三位都是 0 元 0 份），
             *     那份结果送到 Pad 的界面上，由上面那个 filter 收掉。
             *     所以这条检查不会挡住正常 AA —— 穷举 308,592 组
             *     金额>0 的拆分，只有「总额（分）< 人数」时才出现零行。
             *
             *   ★ 「有钱 0 份」不在此列，那是正常生意（只点酒水）；
             *     「0 元有份」由上面那条管。这里只堵两头都是 0 的。
             */
            if ($amt === 0 && $prt === 0) {
                return $fail('zero_allocation');
            }
