...
注入器把 10 张单铺在「注入时刻的前 3~30 分钟」，而 `e2e_pos.php` 的定位断言
走的是 30 分钟窗口。定位断言依赖桌 30 / 23 / 15 / Llevar，其中最老的是
桌 23（注入时已是 18 分钟前）—— **所以注入后约 12 分钟内必须把 e2e 跑完**，
超时后桌 23 会掉出窗口。

**每次跑 e2e 之前重新注入一遍就行**，注入是幂等的（先按 `>= 900000` 清一遍再写）。

`e2e_pos.php` 开头有保鲜检查，过期时会直接停下并指名是哪张桌：

```
✗ 模拟活单已过期：桌 23 已 58 分钟，超出 30 分钟定位窗口
  这不是代码问题，是夹具过期 —— 注入后约 12 分钟内有效。请重新注入再跑
```

没有这道检查时，表现是「桌号 23 期望 4 实得 0」，看上去像聚合逻辑出了 bug，
很容易白查半天代码。
...
php tests/run.php               # 纯逻辑，988 项断言，不需要数据库
php tests/smoke.php --fresh     # 真本地库 + FakePos，622 项断言
