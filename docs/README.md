...
| `tests/` | `run.php` 纯逻辑测试（177 项断言）、`smoke.php` 真库冒烟测试 |
...
| 2 | 10送1 核销时收银员在 POS 端的具体动作（整单折扣 / 改价 0 / 删项） | 待确认 | 03（兜底校验精度） |
| 3 | 只读账号能 `SELECT` 的表范围 | 待确认 | 02 |
| 4 | 2024-08 的 6 天数据丢失原因（是否会复发） | 待了解，降级预案已就位 | 01 §5.3、03 §10 |
| 5 | `meal_item_rule` 初始化：把 `menu_item` 中 `price_1 ≥ 8.00` 的 61 项一次性归类入表 | 实施任务，非决策 | 04 §6.2 |
