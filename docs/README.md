...
| # | 事项 | 状态 | 影响文档 |
|---|---|---|---|
| 1 | **`BOX 1`~`18` / `COMBO S~XL`（22 项，10~65 欧）算不算餐费？参不参与十送一？** | **等待确认** | 03 §5.3/§5.4、04 §6.2 |
...
> **当前无 🔴 阻塞项。** 事项 1 已有安全默认值（不计次、正常积分），不影响开发启动。

### 事项 1 的背景

`BOX 1`~`BOX 18`、`COMBO S/M/L/XL` 共 22 项，价格 10.00~65.00 欧，属 `major_group = 1 (Comida)` / `family_group = 7 (Sushis)`，是寿司拼盘性质的套餐。

- 实测样本中**仅出现在外带订单**（`eat_type = 3`，不积分），但堂食是否可点未确认
- 它们**不在** `major_group = 3 (Menú)`，因此白名单巡检已改为**按价格阈值扫全表**，不再按分组过滤
- 未归类项按安全默认值处理：`is_meal_fee = 0`、`counts_visit = 0`、`earns_points = 1`

需要你确认三个开关怎么设，确认后填入 `meal_item_rule` 表即可。
