...
| 条件 | 含义 | 实例 |
|---|---|---|
| `menu_item_id = -2` | **折扣行**：`actual_price` **恒为负** | `'Dto. -20%'`、`'TARJETA 10+1'` |
| `menu_item_id = -3` | 操作留痕行 | `'**999 Enviado 19:16**'`、`'**555 下单 22:09**'` |
| `menu_item_id = -4` | **支付行**：`actual_price` = **收款额（含待找零）** | `'EFECTIVO'`（现金），实测 `actual_price = 15.00` |
...
`-2` 的四种名称：`Dto. -20%`×54、`CUPON DE 5 EUROS`×4、`Dto%`×4、**`TARJETA 10+1`×1**
...
> 也不是改价 0，而是加一条 `-2` 折扣行。
> 系统据此把该单标记为核销餐（`pos_order.is_redeemed`），不计次不积分 ——
> 否则客人「拿奖励的同时又攒了 5 次」。判定按**可配置的名称模式**
> （`sys_config.redeem_line_patterns`），因为 `Dto. -20%` / `CUPON` 是普通折扣，
> 误判会让正常打折的客人拿不到积分。
>
