...
> 🟡 **待补（不阻塞上线）**：需要 `major_group` / `family_group` 两张表的数据（各几十行）以确定分组名称，进而配置"餐费项"白名单。见 `README.md` 待确认事项 #1。
>
> §3.2 的价格字段判据已能覆盖 AA 显示过滤与免费餐第一层兜底校验；白名单仅用于把兜底校验从「有东西被免了」精确到「被免的确实是套餐主项」，降低误报率。
